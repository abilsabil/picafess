...
ENV_CONFESS_ID = os.getenv("1478417757488939089")
ENV_RIDDLE_ID = os.getenv("1478417854238818487")
...
class RiddleAnswerModal(discord.ui.Modal, title='Kirim Jawaban Riddle'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("✅ Jawaban dihantar!", ephemeral=True)
...
class RiddleView(discord.ui.View):
    ...
    async def win_btn(self, itx, btn):
        if itx.user.id != self.creator_id and not itx.user.guild_permissions.administrator:
            return await itx.response.send_message("Hanya pembuat/Admin boleh menutup ini.", ephemeral=True)
...
class SendConfessModal(discord.ui.Modal, title='Kirim Confess'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not chan: return await interaction.response.send_message("❌ Channel belum ditetapkan!", ephemeral=True)
        config["count"] += 1
        embed = discord.Embed(title=f"💌 PICAFESS #{config['count']}", description=self.content.value, color=DEFAULT_COLOR)
        msg = await chan.send(embed=embed)
        await msg.create_thread(name="Komentar", auto_archive_duration=1440)
        save_data(server_data)
        ...
        await interaction.response.send_message("✅ Dihantar secara anonim!", ephemeral=True)
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.Thread):
        if message.channel.name == "Diskusi & Jawaban Riddle":
        ...
        elif message.channel.name == "Komentar":
...
async def sc(itx, channel: discord.TextChannel):
    ...
    welcome = discord.Embed(title="🌸 Selamat Datang di Picafess!", 
                           description="Gunakan `/picafess` untuk menghantar pesanan anonim.\nSetiap pesanan akan mempunyai thread 'Komentar' yang juga bersifat anonim.", 
                           color=DEFAULT_COLOR)
...
async def sr(itx, channel: discord.TextChannel):
    ...
    welcome = discord.Embed(title="🧩 Selamat Datang di Pica Riddle!", 
                           description="Gunakan `/riddle-setup` untuk memulakan teka-teki.\nKlik butang 'Jawab' atau tulis terus di thread untuk memberi jawapan.", 
                           color=0x2ecc71)
    await channel.send(embed=welcome)
...
async def rs(itx, pertanyaan: str):
    ...
    if not chan: return await itx.response.send_message("❌ Sila set channel dahulu!", ephemeral=True)
    ...
    await itx.response.send_message("✅ Riddle dihantar!", ephemeral=True)
